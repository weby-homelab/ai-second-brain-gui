...
The source distribution produced by setuptools contains generated directories
whose fractional mtimes vary between builds.  Rebuilding the tar stream here
keeps the file bytes and ordinary links intact while removing those host-
...
def _normalize_member(member: tarfile.TarInfo, timestamp: int) -> tarfile.TarInfo:
    ...
    # TarInfo.name and TarInfo.linkname are enough to regenerate the required
    # PAX path fields.  Other PAX fields may contain host-specific timestamps.
