"""
Two-browser end-to-end harness for power-gui.

Simulates two independent browsers (separate cookie jars / sessions) talking to a
LIVE power-gui server and verifies:
...
@pytest.mark.skipif(not _reachable(BASE), reason=f"live power-gui server not reachable at {BASE}")
...
@pytest.mark.skipif(
    ...
    reason="POWER_GUI_E2E_READONLY_URL not set or unreachable",
)
