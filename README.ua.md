...
- ✏️ **Редагування Markdown-нотаток:** Можливість редагувати та зберігати будь-які нотатки безпосередньо з веб-інтерфейсу дашборду. Забезпечено надійний захист від Path Traversal завдяки перевірці шляхів через `validate_path`.
...
Конфігурація керується через Pydantic Settings (префікс `POWER_GUI_`):

```bash
# Опціональний хеш пароля для режиму з авторизацією
