...
[![P.O.W.E.R](https://img.shields.io/badge/P.O.W.E.R-3.7.2--candidate-FF6B6B?style=for-the-badge)](https://github.com/weby-homelab/power-framework)
...
**Кандидат suite:** GUI `0.7.8` проти source-pinned POWER `v3.7.2` контракту `power.application.v2` (Python `>=3.13,<3.15`). Це candidate worktree: digest контейнера, SBOM/provenance, publication і live E2E readback залишаються явними release gates у [`compatibility.json`](compatibility.json). Публічна discovery-поверхня залишається experimental custom discovery; стабільні claims A2A та multi-writer Federation не підтримуються.
...
services:
  power-gui:
    image: webyhomelab/power-gui:0.7.8
...
### Контракт read model POWER 3.7.2
