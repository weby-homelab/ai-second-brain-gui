...
> **Target Application:** `power-gui` (P.O.W.E.R-GUI v0.7.10)
...
1. **Install the exact suite wheel pair into the managed profile:**
   ...
   power integrations install \
     --power-wheel /path/to/power_framework-3.7.4-py3-none-any.whl \
     --gui-wheel /path/to/power_gui-0.7.10-py3-none-any.whl
   power integrations install \
     --power-wheel /path/to/power_framework-3.7.4-py3-none-any.whl \
     --gui-wheel /path/to/power_gui-0.7.10-py3-none-any.whl \
     --apply --approved
   ```
   Planning is read-only; the active managed venv is selected through
   `~/.local/share/power/current/venv`, and launchers are under `~/.local/bin`.
